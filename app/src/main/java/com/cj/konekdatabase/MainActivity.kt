            ...
            .getAsJSONObject(object : JSONObjectRequestListener {
                override fun onResponse(response: JSONObject?) {
                    ...
                    if (jsonArray?.length() == 0){
                        loading.dismiss()
                        Toast.makeText(applicationContext,"Fakultas data is empty, Add the data first",Toast.LENGTH_SHORT)
                            .show()
                        }
                    for(i in 0 until jsonArray?.length()!!){
                        ...
                        if(jsonArray?.length()-1 == i){
                            loading.dismiss()val adapter = RVAdapterFakultas(applicationContext,arrayList)
